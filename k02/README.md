...
## ソースコードの説明
l.79:新しい要素のデータ領域を確保
l.80:用意したデータ領域に新しい市町村データを代入
l.81:用意した要素の次のアドレスに挿入前の先頭アドレスを代入
l.83:要素を挿入後のリストの先頭アドレスを返す

l.107:posの初期化
l.108:リストの先頭アドレスをtopに代入
l.109~116:topにデータがあれば、topのcity.idと探索したいIDが一致した場合、*pCityにtopのcityデータを代入しposの値を返す。一致しなかった場合、次の要素に移動し、posに1を足す。これを繰り返す。
l.117:一致するものがなかった場合、-１を返す


## 出力結果

```
PS C:\Users\bb35319031\プログラミング\2020psp3\k02> ./k02.exe
